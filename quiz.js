...
let quizContent = JSON.parse(quizjson);
...
let currentTime;
let questionCounter;
let questionsList = quizContent.questions;
let middlePart = document.getElementsByClassName('middle')[0];
...
let firstStepPage = document.getElementsByClassName('firstpage')[0];
...
submitAns.addEventListener('click', function () {
    let answer = document.querySelector("input[name=ans]:checked");
    if (answer) {
...
function isCorrectAnswer(currentQuestion, chosenAnswer) {
...
function isAlreadyAnswered(num) {
...
function displayQuestion(number) {
    let currQ = questionsList[number];
